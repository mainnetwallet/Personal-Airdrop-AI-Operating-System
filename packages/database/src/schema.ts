 ...
 * - `agent_runs`, `events`, `memory_entries`, and `tool_registry` are
 *   durable mirrors of the Phase 2 Agent OS Kernel's in-memory state
 *   (@airdrop-os/core). The kernel — not a DB constraint — is the
 *   authoritative enforcer of state transitions, run limits, and
 *   permission checks.
 */
import {
  pgTable,
...
// Phase 2 (Agent OS Kernel): full agent run record, mirroring the
// in-memory AgentRun type/state machine in @airdrop-os/core. This table
// is the durable record of the same data — the kernel is the
// authoritative enforcer of state transitions and limits, not a DB
// constraint.
export const agentRuns = pgTable("agent_runs", {
  ...
  steps: text("steps").notNull().default("0"),
  toolCalls: text("tool_calls").notNull().default("0"),
  retries: text("retries").notNull().default("0"),
  errors: jsonb("errors").notNull().default([]),
  result: jsonb("result"),
  cost: jsonb("cost").notNull().default({ toolCalls: 0, amount: 0 }),
...
// (see @airdrop-os/core MemoryStore); no column here is exempt from
// that rule. `correctionHistory` preserves every prior version of
...
export const toolRegistryTable = pgTable("tool_registry", {
